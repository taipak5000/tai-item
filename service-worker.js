/* ================================================================
   ...
   ================================================================ */
const CACHE_VERSION = 'sky-item-v19'; // v19: 関連ツール一覧（SITE_LINKS）に「再訪精霊データベース」を
// 羽トラッカーの直後へ追加（profiles.jsのSITE_LINKSを更新）
// v18: 連鎖オーバーレイでトリガー自身が別オーバーレイの中に住んでいる場合
// （例: Profileモーダル内のバックアップボタン）、親を閉じた後の復帰先が非フォーカス可能になっても
// フォールバック先が見つからず(trapStackが空)フォーカスが宙に浮く不具合をios-hig.jsで修正
// （#siteDockの先頭ボタンへ安全にフォールバックするようfallbackFocusTarget()を拡張）
// v17: ダッシュボードカレンダーのイベント配色をかわいらしいパステル系に変更
// （profiles.jsのDASH_CAL_BAR_PALETTEと背景色ごとの文字色自動選択ロジックを更新）
// v16: 連鎖オーバーレイ（Aを開いたままBを開いて排他制御でAが自動的に閉じる）でフォーカストラップの
// トリガー捕捉が同一MutationObserverバッチ内で汚染され、Bを閉じた時にBの本来の復帰先ではなくAの
// 取り残されたトリガーへフォーカスが戻ってしまう不具合をios-hig.jsで修正
